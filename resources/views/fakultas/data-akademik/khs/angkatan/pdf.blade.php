    ...
    <div style="text-transform: uppercase; font-family: Arial, Helvetica, sans-serif" style="margin-left: 2%">
        ...
        <table class="text-10"
            ...
            <tr>
                ...
                <td width="3"> : </td>
                <td class="text-pdf text-8 text-upper" style="width: 30%">
                    {{$prodi->fakultas->nama_fakultas}}
                    {{-- {{ isset($item['nim']) ? $item['nim'] : 'NIM Tidak Tersedia' }} --}}
                </td>
            </tr>
            <tr>
    ...
    <div class="row">
        <div class="table-responsive">
            <table id="header" class="text-10" border="1" rules="all" style="width: 100%">
                ...
                {{-- @foreach ($data as $item) --}}
                    @if ($item['khs_transfer'] ->isNotEmpty())
                    {{-- @if($item->khs_transfer->isNotEmpty()) --}}
                        <thead>
                        ...
                        <tbody>
                            @foreach ($item['khs_transfer'] as $d)
                                <tr>
                                    <td width="30" class="text-td text-center">{{ $loop->iteration }}</td>
                ...
                {{-- @foreach ($data as $item) --}}
                    @if ($item['khs_konversi'] ->isNotEmpty())
                    {{-- @if($item->khs_transfer->isNotEmpty()) --}}
                        <thead>
                            <tr style="background-color: #d7d7d7;"> <!-- Warna abu-abu muda -->
                        ...
                        <tbody>
                            @foreach ($item['khs_konversi'] as $d)
                                <tr>
                                    <td width="30" class="text-td text-center">{{ $loop->iteration }}</td>
                                    <td width="80" class="text-td text-center">
                ...
                <tfoot style="border-top: none;">
                    <tr>
                        ...
                        <td width="120" class="text-thead" colspan="3"><strong>{{ $item['akm'] !== NULL ? $item['akm']->sks_semester : 'Tidak Ada AKM' }}</strong></td>
                    </tr>
                    <tr>
                        <td width="327" class="text-start" colspan="3" style="padding: 0.2rem 0.3rem; font-size:8pt;">
                            <strong>Total Kredit Yang Telah Ditempuh</strong>
                        </td>
                        <td width="120" class="text-thead" colspan="3"><strong>
                            {{ $item['akm'] ? $item['akm']->sks_total : 'Tidak Ada AKM' }}
                        </strong></td>
                    </tr>
                    <tr>
                        <td width="327" class="text-start" colspan="3" style="padding: 0.2rem 0.3rem; font-size:8pt;">
                        ...
                        <td width="120" class="text-thead" colspan="3"><strong>
                            {{ $item['akm'] ? $item['akm']->ips : 'Tidak Ada AKM' }}
                        </strong></td>
                    </tr>
                    <tr>
                        <td width="327" class="text-start" colspan="3" style="padding: 0.2rem 0.3rem; font-size:8pt;">
                        ...
                        <td width="120" class="text-thead" colspan="3"><strong>
                            {{ $item['akm'] ? $item['akm']->ipk : 'Tidak Ada AKM' }}
                        </strong></td>
                    </tr>
                </tfoot>
            </table>
