...
<div class="content-header">
    <div class="d-flex align-items-center">
        <div class="me-auto">
            <h3 class="page-title">Daftar Mata Kuliah Mahasiswa</h3>
                ...
                <div class="box-body text-center">
                    <div class="table-responsive">
                        <div id="khsDiv" hidden>
                            ...
                            <h3 class="text-center">Daftar Mata Kuliah Mahasiswa</h3>
                            <table style="width:100%" class="mb-3">
                                ...
                                <tr>
                                    ...
                                    <td class="text-start align-middle" style="width: 18%">NAMA JENJANG</td>
                                    <td>:</td>
                                    <td class="text-start align-middle" id="jenjangKrs"
                                        style="width: 30%; padding-left: 10px"></td>
                                </tr>
                                <tr>
 ...
 function getKrs()
    {
        ...
        if(nim == ''){
            swal({
                title: "Peringatan!",
                text: "Nomor Induk Mahasiswa harus diisi!",
                type: "warning",
                buttons: {
                    confirm: {
            ...
            $.ajax({
                url: '{{route('fakultas.yudisium.khs-transkrip-data')}}',
                type: 'GET',
                data: {
                    nim: nim,
                },
                success: function(response){
                    ...
                    $('#jenjangKrs').text(
                        response.riwayat.prodi.nama_jenjang_pendidikan.toUpperCase()
                    );
                    $('#prodiKrs').text(
                        response.riwayat.prodi.nama_program_studi.toUpperCase()
                    );
                    ...
                    var approved = 0;
                    var no = 1;
                    // map untuk menghitung jumlah MK
                    // =====================
                    // HITUNG DUPLIKAT MK
                    // =====================
                    let mkCounter = {};
                    ...
                    response.nilai.forEach(item => {
                        let key = item.kode_mata_kuliah ?? item.nama_mata_kuliah;
                        if (key) {
                            mkCounter[key] = (mkCounter[key] || 0) + 1;
                        }
                    });

                    // dari konversi (opsional tapi aman)
                    if (response.konversi && response.konversi.length > 0) {
                        response.konversi.forEach(item => {
                            let key = item.id_mata_kuliah ?? item.matkul?.kode_mata_kuliah;
                            if (key) {
                                mkCounter[key] = (mkCounter[key] || 0) + 1;
                            }
                        });
                    ...
                    let trClass = '';
                    let key = khs_reg.id_mata_kuliah ?? khs_reg.kode_mata_kuliah;

                    // prioritas MERAH
                    if (khs_reg.nilai_huruf === 'F' || khs_reg.nilai_huruf === null) {
                        trClass = 'bg-danger';
                    }
                    // KUNING jika MK duplikat
                    else if (mkCounter[key] > 1) {
                        trClass = 'bg-warning';
                    }
                        ...
                        $('#khs-regular tbody').append(`
                            <tr class="${trClass}">
                                ...
                                <td class="text-start align-middle"  style="text-transform: uppercase;">${khs_reg.nama_mata_kuliah}</td>
                    ...
                    if (response.konversi.length > 0) {
                        ...
                        response.konversi.forEach(function(konversi, index){
                            let trClass = '';
                            let key = konversi.id_mata_kuliah ?? konversi.matkul?.kode_mata_kuliah;
                            ...
                            $('#khs-regular tbody').append(`
                                <tr class="${trClass}">
                                    ...
                                    <td class="text-start align-middle"  style="text-transform: uppercase;">${konversi.nama_mata_kuliah}</td>
                        ...
                        response.transfer.forEach(function(transfer, index){
                            $('#transferTable tbody').append(`
                                <tr>
                                    ...
                                    <td class="text-start align-middle" style="text-transform: uppercase;">${transfer.nama_mata_kuliah_diakui}</td>
