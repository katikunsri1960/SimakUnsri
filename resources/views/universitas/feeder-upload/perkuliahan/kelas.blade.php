...
@section('title')
FEEDER UPLOAD - KELAS KULIAH
@endsection
@section('content')
<div class="content-header">
    <div class="d-flex align-items-center">
        <div class="me-auto">
            <h3 class="page-title">FEEDER UPLOAD - KELAS KULIAH</h3>
                                ...
                                <div class="col-md-6">
                                    <div class="row">
                                        <button type="submit" class="btn btn-primary btn-sm" disabled id="buttonSubmitForm"> <i
                                                class="fa fa-upload me-3"></i>Upload Kelas</button>
                                    </div>
                                </div>
                            </form>
                            <div class="row mt-3">
...
<script>
    function getData(){
        ...
        $.ajax({
            url: "{{ route('univ.feeder-upload.perkuliahan.kelas.data') }}",
            type: 'GET',
            data: {
                id_prodi: id_prodi,
            ...
            success: function(response) {
                ...
                $.each(data, function(i, item) {
                    ...
                    html += '<td>' + status + '</td>';
                    html += '<td class="text-center">' + item.kode_matkul + '</td>';
                    html += '<td>' + item.nama_matkul + '</td>';
                    html += '<td class="text-center">' + item.nama_kelas_kuliah + '</td>';
                    html += '<td class="text-center">' + item.nm_semester + '</td>';
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            var eventSourceUrl = "{{ route('univ.feeder-upload.perkuliahan.kelas.upload') }}" + "?prodi=" + id_prodi + "&semester=" + id_semester;
