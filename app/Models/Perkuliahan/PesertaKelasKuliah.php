...
class PesertaKelasKuliah extends Model
{
    ...
    public function approve_all($id_reg)
    {
        ...
        if ((!$transkrip || $transkrip->ipk === null) && ($riwayat_pendidikan->id_periode_masuk != $semester_aktif->id_semester || $riwayat_pendidikan->id_periode_masuk + 1 != $semester_aktif->id_semester)) {
            return [
                'status' => 'error',
                    ...
                    if($beasiswa){
                        if($beasiswa->id_pembiayaan == '3'){
                            $peserta = AktivitasKuliahMahasiswa::where('id',$akm_aktif->id)->update([
                                ...
                                'ips'=> '0.00',
                                'ipk' => (!$transkrip && ($riwayat_pendidikan->id_periode_masuk == $semester_aktif->id_semester || $riwayat_pendidikan->id_periode_masuk + 1 == $semester_aktif->id_semester )) ? 0 : ($transkrip?->ipk ?? 0),
                                'sks_semester'=> $total_sks,
                                'sks_total' => (!$transkrip && ($riwayat_pendidikan->id_periode_masuk == $semester_aktif->id_semester || $riwayat_pendidikan->id_periode_masuk + 1 == $semester_aktif->id_semester))  ? 0 : ($transkrip?->total_sks ?? 0),
                                'biaya_kuliah_smt' => $ukt_mahasiswa,
                                'id_pembiayaan' => 3,
                                'status_sync' => 'belum sync',
                            ]);
                        }else if($beasiswa->id_pembiayaan == '2' && ($pembayaran_manual || $tagihan)){
                            $peserta = AktivitasKuliahMahasiswa::where('id',$akm_aktif->id)->update([
                                'feeder' => 0,
                                'id_registrasi_mahasiswa' => $id_reg,
                                'nim' => $riwayat_pendidikan->nim,
    ...
    public function approve_all_univ($id_reg)
    {
        $semester_aktif = SemesterAktif::first();
        $data = PesertaKelasKuliah::with(['kelas_kuliah', 'kelas_kuliah.matkul'])
                ->whereHas('kelas_kuliah', function($query) use ($semester_aktif) {
        ...
        try {
            ...
            if($akm_aktif){
                ...
                if($data_mbkm > 0 || $data_mbkm_eksternal > 0){
                    ...
                    }else if($pembayaran_manual || $tagihan){
                        $peserta = AktivitasKuliahMahasiswa::where('id',$akm_aktif->id)->update([
                            'feeder' => 0,
                            'id_registrasi_mahasiswa' => $id_reg,
                            'nim' => $riwayat_pendidikan->nim,
    ...
    public function approve_v2($id_reg)
    {
        ...
        if ($beasiswa && $beasiswa->tanggal_akhir_beasiswa > now()) {
            $bayar = [
        ...
        try {
            ...
            AktivitasKuliahMahasiswa::updateOrCreate(['id_registrasi_mahasiswa' => $id_reg, 'id_semester' => $semester_aktif->id_semester], [
                ...
                'id_pembiayaan' => $bayar['id_pembiayaan'],
                'status_sync' => 'belum sync',
            ]);
