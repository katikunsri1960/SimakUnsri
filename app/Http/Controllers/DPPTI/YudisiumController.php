...
class YudisiumController extends Controller
{
    ...
    public function peserta()
    {
        ...
        return view('dppti.yudisium.peserta.index', [
            'fakultas' => $fakultas,
            'prodi' => $prodi,
            'periode' => $periode,
    ...
    public function peserta_formulir(Wisuda $id)
    {
        ...
        $pdf = Pdf::loadView('dppti.yudisium.peserta.formulir', [
            'riwayat'    => $riwayat,
            'biodata'    => $biodata,
            'aktivitas'  => $aktivitas,
            'pt'         => $pt,
    ...
    public function peserta_data_approved_transkrip(Request $request)
    {
    ...
    public function peserta_data(Request $request)
    {
        ...
        $data = Wisuda::join('riwayat_pendidikans as r', 'r.id_registrasi_mahasiswa', 'data_wisuda.id_registrasi_mahasiswa')
                ...
                // ->where('pw.periode', $req['periode'])
                ->where('data_wisuda.finalisasi_data', 1)
                ->whereNot('data_wisuda.approved_wisuda', 3)
                ->select(
                    'data_wisuda.*',
                    // 'r.nim',                 // ✅ WAJIB
                    // 'r.id_prodi',            // ✅ WAJIB
    ...
    public function approve(Request $request, $id)
    {
        ...
        if (!$wisuda->tgl_sk_yudisium) {
            DB::rollBack();
            return response()->json([
                'status' => 'error',
                'message' => 'Tanggal SK Yudisium tidak boleh kosong!',
            ], 422);
        }
        ...
        try {
            ...
            LulusDo::create([
                ...
                'no_seri_ijazah' => in_array($riwayatPendidikan->prodi->id_jenjang_pendidikan, ['31', '32', '37']) ? NULL : '-', // Conditional logic //PERLU DIUBAH
                'tgl_create' => now(),
                'sks_diakui' => $wisuda->sks_diakui ?? null,
                'jalur_skripsi' => NULL,
                'judul_skripsi' => $wisuda->aktivitas_mahasiswa->judul,
                'bln_awal_bimbingan' => NULL,
                'bln_akhir_bimbingan' => NULL,
                'sk_yudisium' => $wisuda->no_sk_yudisium,
                'tgl_sk_yudisium' => $wisuda->tgl_sk_yudisium,
                'ipk' => $wisuda->ipk,
                'sert_prof' => $riwayatPendidikan->prodi->id_jenjang_pendidikan === '31' ? NULL : NULL, // Conditional logic //PERLU DIUBAH
            ...
            // dd(LulusDo::where('id_registrasi_mahasiswa', $wisuda->id_registrasi_mahasiswa)->first());
    ...
    public function decline(Request $request, $id)
    {
        ...
        try {
            ...
            |--------------------------------------------------------------------------
            | Asumsi:
            | - tabel aktivitas_mahasiswas / lulus_do / status_kelulusan
            | - sesuaikan model & field sesuai struktur database Anda
            |--------------------------------------------------------------------------
            */
            $lulusDo = LulusDo::where('id_registrasi_mahasiswa', $wisuda->id_registrasi_mahasiswa)->first();

            if ($lulusDo) {

                // Jika sudah sync feeder, tidak boleh dihapus
                if ($lulusDo->feeder == 1) {
                    DB::rollBack();

                    return response()->json([
                        'status' => 'error',
                        'message' => 'Gagal membatalkan! Data Lulus/DO sudah tersinkron ke feeder PDDikti.',
                    ]);
                }

                // Jika belum sync feeder, hapus data
                if ($lulusDo->feeder == 0) {
                    $lulusDo->delete();
                }
            }
...
//  dd($data);
        ...
        $data['id_semester'] = substr($semesterAktif->id_semester, -1) == '3'
    ...
    {
        return view('dppti.yudisium.perbaikan-data.index');
    }
    ...
    public function ijazah(Request $request)
    {
        ...
        return view('dppti.yudisium.ijazah.index', [
            'fakultas' => $fakultas,
            'periode' => $periode,
            'prodi' => $prodi,
        ]);
    ...
    public function ijazah_download_excel(Request $request)
    {
        ...
        $query = Wisuda::with('predikat_kelulusan')
                ...
                ->select('data_wisuda.*', 'f.nama_fakultas', 'p.nama_program_studi as nama_prodi', 'p.kode_program_studi as kode_prodi', 'p.nama_jenjang_pendidikan as jenjang', 'r.nama_mahasiswa as nama_riwayat', 'r.nim',
    ...
    public function ijazah_download_pdf(Request $request)
    {
        ...
        // dd($dekan, $rektor);
                                ...
                                // dd($data[0]->wisuda_ke);
        $pdf = PDF::loadview('dppti.yudisium.ijazah.pdf', [
            'data' => $data,
            'kode_univ' => $kode_univ,
            'fakultas' => $fakultas,
            'rektor' => $rektor,
    ...
    // {
    //     return view('dppti.yudisium.transkrip.index');
    // }
    ...
    public function transkrip_download_pdf(Request $request)
    {
        ...
        // $pdf = PDF::loadview('dppti.yudisium.transkrip.pdf', [
        //     'data' => $data,
        //     'kode_univ' => $kode_univ,
        //     'fakultas' => $fakultas,
        //     'wr1' => $wr1,
    ...
    public function album(Request $request)
    {
        ...
        return view('dppti.yudisium.album.index', [
            'fakultas' => $fakultas,
            'periode' => $periode,
            'prodi' => $prodi,
        ]);
    ...
    public function album_download_pdf(Request $request)
    {
        ...
        $pdf = PDF::loadview('dppti.yudisium.album.pdf', [
            'data' => $data,
    ...
    {
        return view('dppti.yudisium.usept.index');
    }
}
