...
class BiodataController extends Controller
{
    ...
    public function index_rev()
    {
        ...
        // Ambil data RiwayatPendidikan dengan relasi
        $data = RiwayatPendidikan::with(['biodata', 'pembimbing_akademik'])
        ...
        // Ambil data wilayah
        $cek_id_wil = $data->biodata->id_wilayah ?? null;
        $id_induk_wilayah = $data->biodata->id_induk_wilayah ?? null;

        $kab_kota = Wilayah::where('id_wilayah', $id_induk_wilayah)
            ->where('id_level_wilayah', 2)
            ->pluck('nama_wilayah')
            ->first();
        $id_kab_kota = Wilayah::where('id_wilayah', $id_induk_wilayah)
            ->where('id_level_wilayah', 2)
            ->pluck('id_induk_wilayah')
            ->first();
